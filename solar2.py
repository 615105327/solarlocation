...
def returncvs(left_upper_long,left_upper_lati,left_lower_long,left_lower_lati,right_upper_long,right_upper_lati,right_lower_long,right_lower_lati):
	...
	with myfile:
		...
		writer.writerow(['left_upper_long','left_upper_lati','left_lower_long','left_lower_lati','right_upper_long','right_upper_lati','right_lower_long','right_lower_lati'])
		for i in range (3):
			for j in range(133):
				writer.writerow([left_upper_long+x*j,left_upper_lati+y*i,left_upper_long+x*j,left_upper_lati+y*(i+1),left_upper_long+x*(j+1),left_upper_lati+y*i,left_upper_long+x*(j+1),left_upper_lati+y*(i+1)])
...
lines = cv2.HoughLinesP(maskedImg,rho = 1,theta = 1*np.pi/180,threshold = 100,minLineLength = 10,maxLineGap = 2000)
N = lines.shape[0]
xlu = lines[1][0][0]
ylu = lines[1][0][1]    
xrd = lines[1][0][2]
